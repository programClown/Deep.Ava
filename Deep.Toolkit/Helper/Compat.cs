...
using Deep.Toolkit.Models.FileInterfaces;
using Semver;
...
public class Compat
{
    ...
    public static string GetExecutableName()
    {
        if (IsLinux)
        {
            ...
            if (string.IsNullOrEmpty(appImage)) throw new Exception("Could not find APPIMAGE environment variable");
    ...
    public static string GetAppName()
    {
        ...
        if (!IsMacOS) return GetExecutableName();
    ...
    public static string GetEnvPathWithExtensions(params string[] paths)
    {
        ...
        if (string.IsNullOrEmpty(currentPath)) return string.Join(PathDelimiter, paths);
