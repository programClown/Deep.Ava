...
public static partial class HardwareHelper
{
    ...
    private static IEnumerable<GpuInfo> IterGpuInfoLinux()
    {
        ...
        foreach (var line in gpuLines)
        {
            ...
            if (match.Success) memoryBytes = ulong.Parse(match.Groups[1].Value) * 1024 * 1024;
    ...
    private static IEnumerable<GpuInfo> IterGpuInfoMacos()
    {
        ...
        foreach (var (i, videoController) in HardwareInfo.VideoControllerList.Enumerate())
        {
            ...
            if (Compat.IsArm) gpuMemoryBytes = GetMemoryInfoImplGeneric().TotalPhysicalBytes;
    ...
    {
        if (Compat.IsWindows) return IterGpuInfoWindows();

        if (Compat.IsLinux)
        {
            // Since this requires shell commands, fetch cached value if available.
            if (cachedGpuInfos is not null) return cachedGpuInfos;
    ...
    public static bool PreferRocm()
    {
        return !HasNvidiaGpu() && HasAmdGpu() && Compat.IsLinux;
    }

    // Set DirectML for default if AMD and Windows
    public static bool PreferDirectML()
    {
        return !HasNvidiaGpu() && HasAmdGpu() && Compat.IsWindows;
    }
    ...
    {
        return Compat.IsWindows ? GetMemoryInfoImplWindows() : GetMemoryInfoImplGeneric();
    }
    ...
    private static MemoryInfo GetMemoryInfoImplWindows()
    {
        ...
        if (!GlobalMemoryStatusEx(ref memoryStatus)) throw new Win32Exception(Marshal.GetLastWin32Error());
    ...
    {
        return Compat.IsWindows ? Task.FromResult(GetCpuInfoImplWindows()) : GetCpuInfoImplGenericAsync();
    }
    ...
    private static Task<CpuInfo> GetCpuInfoImplGenericAsync()
    {
        return Task.Run(() =>
        {
            ...
            if (HardwareInfo.CpuList.FirstOrDefault() is not { } cpu) return default;

            var processorCaption = cpu.Caption.Trim();

            // Try name if caption is empty (like on macos)
            if (string.IsNullOrWhiteSpace(processorCaption)) processorCaption = cpu.Name.Trim();
