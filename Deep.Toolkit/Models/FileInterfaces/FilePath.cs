...
public partial class FilePath : FileSystemPath, IPathObject
{
    ...
    [JsonIgnore] public string NameWithoutExtension => Path.GetFileNameWithoutExtension(Info.Name);
    ...
    [JsonIgnore] public bool Exists => Info.Exists;
    ...
    public async Task<FilePath> MoveToWithIncrementAsync(FilePath destinationFile, int maxTries = 100)
    {
        ...
        {
            if (!targetFile.Exists) return await MoveToAsync(targetFile).ConfigureAwait(false);
    ...
    public static implicit operator FilePath([Localizable(false)] string path)
    {
